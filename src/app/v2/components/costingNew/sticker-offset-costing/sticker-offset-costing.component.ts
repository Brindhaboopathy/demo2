...
import { StickerOffsetCosting } from 'src/app/v2/components/costingNew/model/sticker-offset-costing';
...
@Component({
  selector: 'app-sticker-offset-costing',
  templateUrl: './sticker-offset-costing.component.html',
  styleUrls: ['./sticker-offset-costing.component.css'],
})
export class StickerOffsetCostingComponent implements OnInit {
  //flag to render the page for creation / updation
  isNew: boolean = true;

  costing: StickerOffsetCosting;
  ...
  technicaldetailsReadOnly: boolean = false;
  ...
  constructor(
    ...
    this.costing = new StickerOffsetCosting();
    ...
    if (
      ...
      console.log(this.isSuperUser, this.accessType);
    }
  ...
  userAccessControl() {
    if (
      this.accessType != 'Administrator' &&
      this.accessType != 'Sample Head1' &&
      this.accessType != 'Sample Head2' &&
    ...
    if (
      ...
      (this.accessType == 'Sample Head1' && this.costing.status < 1) ||
    ...
    if(this.accessType == 'Sample Head2'){
     this.technicaldetailsReadOnly = true;
    }
  }

  fetchExecutiveId() {
    //this id will be used to set the executiveId field of stickerOffsetCosting model while saving for the first time
  ...
  fetchExistingCosting() {
    this.costingService
      .getStickerOffsetByid(this.route.snapshot.params['id'])
      .subscribe(
        (data) => {
          this.costing = data;
          ...
          if (this.costing.attachmentExtension != 'pdf') {
            this.costingService
              .getStickerOffsetAttachmentImage(
  ...
  saveCostingInputByExecutive() {
    ...
    if (this.validateExecutiveInput()) {
      this.costingService.createStickerOffset(this.costing).subscribe(
        (data) => {
          if (data) {
            ...
            alert(
              'R - Error while saving - StickerOffset costing sheet. Please contact the administrator.'
            );
            console.log(
        ...
        (err) => {
          alert(
            'Error while saving - StickerOffset costing sheet. Please contact the administrator'
          );
          console.log(err);
          this.disableSaveBtn = false;
        }
  ...
  updateCostingInputByExecutive() {
    if (this.validateExecutiveInput()) {
      this.costingService.updateStickerOffset(this.costing).subscribe(
        (data) => {
          if (data) {
  ...
  sendForApprovalBySampleHead2() {
    if (this.validateSampleHeadInput()) {
      //updating the status
      this.costing.status = 11;
  ...
  initializeCostingApprovalDetails(forCustomer?: boolean) {
    ...
    this.costingApprovalDetails.trimType = 'Sticker-Offset';
    ...
    if (!forCustomer) {
      this.costingApprovalDetails.approvalLink = `${Configuration.domainURL}header/login/costingApproval/Sticker-Offset--${this.costing.id}`;
    } else {
      this.costingApprovalDetails.approvalLink = `${Configuration.domainURL}header/login/costingApprovalForCustomer/Sticker-Offset--${this.costing.id}`;
    }
  }
  ...
  downloadPdf() {
    ...
    const costingModal = document.getElementById(
      'pdf-container-sticker-offset'
    );
    const options = {
      background: 'white',
      scale: 2,
      ...
      .subscribe(
        (data) => {
          ...
          if (data) {
            ...
            //update the costing status on MStickerOffsetCosting and MCommonDetailsCosting
            this.costingService.updateStickerOffset(this.costing).subscribe(
              (data) => {
                if (data) {
                  this.snackBarService.showSuccessSnack(
  ...
  approveByCustomer = async () => {
    ...
    if (flag) {
      try {
        let res = await this.costingService.approveStickerOffsetByCustomer(
          this.costing.id
        );
        if (res) {
          this.snackBarService.showSuccessSnack(
  ...
  rejectByCustomer = async () => {
    ...
    if (reason != '' && reason != null) {
      try {
        let res = await this.costingService.rejectStickerOffsetByCustomer(
          this.costing.id,
          reason
        );
        if (res) {
  ...
  createDraftOrder() {
    ...
    if (this.costingService.createDraftOrderStickerOffset(this.costing.id)) {

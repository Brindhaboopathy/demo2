...
export class CostingListComponent implements OnInit {
  ...
  showActiveCosting() {
      ...
      this.userService
        ...
        .subscribe(
          (data: Userprofile) => {
            ...
            this.costingService
              .getAllCommonDetailsForSampleHead1(data.trimTypes)
              .subscribe(
                (data) => {
                  data.sort(this.compare);
                  this.commonDetails = data.filter(item => item.status == 0);
                },
                (err) => {
    ...
    else if (this.accessType == 'Sample Head2') {
      this.userService
        .getUserByLoginId(localStorage.getItem('emailToken'))
        .subscribe(
          (data: Userprofile) => {

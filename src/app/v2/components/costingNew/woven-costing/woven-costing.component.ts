...
export class WovenCostingComponent implements OnInit {
  ...
  public costingdetailsReadOnly: boolean = false;
  ...
  constructor(
    ...
    if (this.route.snapshot.params['id'] == 'new') {
      ...
      else if (this.accessType == 'Sample Head1') {
        this.costing.approverId = localStorage.getItem('token');
      }
    } else {
      this.isNew = false;
    }
  ...
  userAccessControl() {
    ...
    if(
      this.accessType == 'Sample Head2'){
        this.costingdetailsReadOnly =true;
  ...
  sendToSampleHead2() {
    ...
    // this.costingService.updateWoven(this.costing).subscribe(
    //   (data) => { if (data) { this.snackBarService.showSuccessSnack('Send For Sample Head 2 Approval') } });
  }
  ...
  updateCostingInputBySampleHead1(approvalReq?: boolean) {
        ...
        (data) => {
          if (data) {
            if (this.imageUpdateFlag) {
              ...
              this.snackBarService.showSuccessSnack(
                `Costing sheet ${approvalReq ? 'approval request sent' : 'updated'
                } successfully`
              );
              this.back();
            }
